...
import { transactions } from "../../settings/consts";
import { useNavigate } from "react-router-dom";
import TransactionDetailModal from "../../common/models/TransactionDetailModal";

const TransactionList = ({ handleCurrentWindow }) => {
  ...
  useEffect(() => {
    handleCurrentWindow("Transaction List");
  }, []);
      ...
      <Grid container justifyContent="center">
        <Grid item xs={12} sm={12} md={12}>
          <Grid item xs={12} sm={12} md={12} className="mb-3 d-flex">
            ...
            <Grid
              item
              className="flex-grow-1 mx-2 d-flex justify-content-end align-middle"
            >
                      ...
                      <tr key={transaction.id}>
                        <td>
                          <Checkbox
                            ...
                            onChange={(event) =>
                              handleRowCheckboxChange(event, 1)
                            }
                          />
                        </td>
                        <td>{transaction.type}</td>
                        <td>{transaction.vono}</td>
                        <td>{transaction.date}</td>
                        <td>{transaction.cname}</td>
                        <td>{transaction.vamt}</td>
                        <td>
                          <IconButton
                            type={faTrash}
                            bgColor={"#e2001a"}
